...
sub _draw_single_person_page {
  ...
  $self->_draw_row( 'nickname', $self->{target_person}->get_name()->get_nickname() )
  	if(defined $self->{target_person}->get_name()->get_nickname());
  $self->_draw_row( "place_of_birth", $self->{target_person}->get_place_of_birth()->toString() )
    if(defined $self->{target_person}->get_place_of_birth());
  $self->_draw_row( "place_of_death", $self->{target_person}->get_place_of_death()->toString() )
    if(defined $self->{target_person}->get_place_of_death());
  $self->_draw_row( "cemetery", $self->{target_person}->get_cemetery()->toString() )
    if(defined $self->{target_person}->get_cemetery());
  $self->_draw_row( "schools", join(', ', @{$self->{target_person}->get_schools()}) )
    if(defined $self->{target_person}->get_schools());
  $self->_draw_row( "jobs", join(', ', @{$self->{target_person}->get_jobs()} ) )
    if(defined $self->{target_person}->get_jobs());
  $self->_draw_row( "work_places", join(', ', @{$self->{target_person}->get_work_places()} ) )
    if(defined $self->{target_person}->get_work_places());
  $self->_draw_row( "places_of_living", join(', ', map {$_->toString()} 
    @{$self->{target_person}->get_places_of_living()}) )
    if(defined $self->{target_person}->get_places_of_living());
  
  $self->_draw_row( "father", $self->aref_tree(
    $self->{target_person}->get_father()->get_name()->get_long_name(), $self->{target_person}->get_father(), 0) )
    if ( defined $self->{target_person}->get_father() );
  $self->_draw_row("mother", $self->aref_tree(
    $self->{target_person}->get_mother()->get_name()->get_long_name(), $self->{target_person}->get_mother(), 0))
  ...
  $self->_print_people_list( "children", $self->{target_person}->get_children() )
  ...
  if ( defined $people_array_r && @{$people_array_r} > 0) {
    ...
    foreach my $person (@{$people_array_r}) {
      if ( $person != $self->{target_person} 
      		&& $person != $Person::unknown_male && $person != $Person::unknown_female) {
